...
class CModel extends CObject
{
	...
	var $validateitem	= array();	//ROCOZ川西
	...
	function convert_string(&$data) {
		...
		foreach ($data as $key => $value) {
			if (array_key_exists($key, $this->convert_list)) {

				if (!empty($value)) {
					if (!is_array($value)){
						$option     = $this->convert_list[$key];
						$data[$key] = mb_convert_kana($value,$option,"utf-8");
					}
				}
		...
		foreach ($data as $key => $value) {
			if (array_key_exists($key, $this->validatefunc)) {
				...
				foreach ($funcs as $i => $func) {
					...
					list($func,$opt) = explode('/',$func);
				
				
					if (method_exists($validater, $func)) {
					
						if ($opt == "") {
							$chk = $validater->$func($value);
						}else{
							$chk = $validater->$func($value,$opt);
						}
					
						if (!$chk) {
							if (array_key_exists($key, $this->validatemsg)) {
								$this->validateresult[$key]	= $errors[$i];
								$ret .= $errors[$i];
							}
						}
	...
	function validatemsg_dx($data) {
	
		global $CONF_VALIDATE_MESSAGE;
		...
		foreach ($data as $key => $value) {
			if (array_key_exists($key, $this->validatefunc)) {
				...
				foreach ($funcs as $i => $func) {
					...
					list($func,$len) = explode('/',$func);
				
					$format_string = $CONF_VALIDATE_MESSAGE[$func];

					$format_string = str_replace("<%item%>", $item, $format_string);
					$format_string = str_replace("<%len%>", $len, $format_string);
					$errors[$i]    = $format_string;
					...
					if (method_exists($validater, $func)) {
						...
						if ($len == "") {
							...
							$chk = $validater->$func($value,$len);
						}
					
						if (!$chk) {
							if (array_key_exists($key, $this->validateitem)) {
								$this->validateresult[$key]	= $errors[$i];
								$ret .= $errors[$i];
							}
						}
	...
	function validatemsg_dx_front($data) {
	
		global $CONF_VALIDATE_MESSAGE_FRONT;
		...
		foreach ($data as $key => $value) {
			if (array_key_exists($key, $this->validatefunc)) {
				...
				foreach ($funcs as $i => $func) {
					...
					$format_string = $CONF_VALIDATE_MESSAGE_FRONT[$func];
